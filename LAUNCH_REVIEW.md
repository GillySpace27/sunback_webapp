...
- [x] **B1 done:** Printful token **revoked at the vendor 2026-07-15**. Git history purged with `git-filter-repo --path .env --invert-paths` and force-pushed across all 12 branches on GitHub 2026-07-15 — a normal `git clone` no longer surfaces `.env` on any branch. **Residual, low-severity exposure:** GitHub's own `refs/pull/1..9/head` refs (frozen PR snapshots, not touched by force-push — a GitHub platform limitation, not a git one) still resolve to the pre-purge commit `e36a21f` and its `.env`. Fetchable only by someone who explicitly requests `refs/pull/N/head` — not visible via a normal clone, browse, or the repo's current branches. Since the key itself is already revoked and dead, this is informational, not urgent; full elimination requires the repo owner to file a GitHub Support request for a cache purge (github.com/contact) — not doable via git.
...
- [x] **Analytics installed:** GA4 live (`G-YQGH255Z02`, dedicated "My Heliograph" property). Sentry live (personal org "sunny-days", project `my-heliograph`, DSN deployed 2026-07-15) — client init confirmed on myheliograph.com (`window.Sentry` loaded, `_sentryInited` true, a test `captureMessage` returned a real event ID). Delivery to the Sentry dashboard should be spot-checked by the operator (Issues feed → "Launch-verification test event from Claude", safe to delete).
