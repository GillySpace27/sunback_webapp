...
def test_manifest_persist_preserves_flat_keys(tmp_path=None):
    """Regression guard for the 2026-08-15 clobber: the flat per-product keys
    are the store's fallback mockups and must survive every persist."""
    import tempfile
    tmp = pathlib.Path(tmp_path or tempfile.mkdtemp())
    main.DEFAULT_MOCKUPS_MANIFEST = tmp / "default_mockups.json"
    # Mixed: a nested wl key + a legacy flat product key.
    ...
    assert "171" in loaded and "poster_matte" in loaded          # BOTH survive
    assert loaded["poster_matte"]["url"] == "legacy-flat"
