...
# ────────────────────────────────────────────────
def _select_representative_images(images: list, variants: list) -> Optional[list]:
    """Return a NEW images list with `is_selected_for_publishing` True only for
    one representative enabled variant's mockups (plus any variant-agnostic
    images). This collapses the N-per-size duplicate gallery down to a single
    clean set. Returns None when there's nothing worth trimming (so the caller
    leaves Printify's default all-selected behavior untouched)."""
    if not images or len(images) <= 4:
        return None
    enabled_ids = [v.get("id") for v in (variants or []) if v.get("is_enabled")]
    if not enabled_ids:
        return None
    enabled_set = set(enabled_ids)
    # Prefer a variant that owns a default mockup (so the kept set keeps a
    # sensible primary image); else fall back to the first enabled variant.
    target = None
    for img in images:
        if img.get("is_default"):
            for vid in (img.get("variant_ids") or []):
                if vid in enabled_set:
                    target = vid
                    break
    ...
    for img in images:
        ...
        keep = (not vids) or (target in vids)
        ni = dict(img)
        ni["is_selected_for_publishing"] = bool(keep)
        if keep:
            kept += 1
        out.append(ni)
    # Don't act if it would change nothing or empty the gallery.
    if kept == 0 or kept == len(images):
        return None
    return out


def _trim_product_mockups(shop_id: str, product_id: str, product_data: dict) -> None:
    """Deselect duplicate per-size mockups on a freshly created product so only
    one representative set publishes to Shopify. No-op (raises nothing fatal) if
    the catalog data isn't shaped as expected — caller wraps this in try/except."""
    images = product_data.get("images") or []
    variants = product_data.get("variants") or []
    if not images:
        # Mockups can lag product creation by a moment; fetch once.
        resp = _printify_request(
            "GET",
        ...
        if resp.status_code == 200:
            ...
            variants = pd.get("variants") or variants
    new_images = _select_representative_images(images, variants)
    if not new_images:
        _log(f"[checkout] mockup trim: nothing to trim ({len(images)} imgs)")
        return
    resp = _printify_request(
        "PUT",
        f"{PRINTIFY_BASE}/shops/{shop_id}/products/{product_id}.json",
    ...
    if resp.status_code not in (200, 201):
        raise Exception(f"trim PUT {resp.status_code}: {resp.text[:200]}")
    kept = sum(1 for i in new_images if i.get("is_selected_for_publishing"))
    _log(f"[checkout] trimmed mockups: {kept}/{len(new_images)} images selected for publish")
...
def _do_checkout_sync(
    ...
    # ── Step 2b: Trim mockups ──
    # Printify auto-generates one mockup per (scene × variant). With N enabled
    # sizes that's N near-identical images (same art, different size label), so
    # the Shopify gallery ends up cluttered with ~3×N shots. Deselect all but a
    # single representative variant's mockups so the buyer sees one clean set.
    # Fail-safe: any error here just leaves all images selected (prior behavior).
    try:
        _trim_product_mockups(shop_id, product_id, product_data)
    except Exception as e:
        _log(f"[checkout] mockup trim skipped (continuing): {e}")
